...
module.exports = {
    ...
    allProducts: (req, res) => {
        connection.query('SELECT * from product where isBought = 0 and isDeleted = 0', async (err, rows) => {
            const response = await graphql(productsSchema, productsQuery, {produts: rows});
            res.setHeader('Content-Type', 'application/json');
            res.status(200).send(JSON.stringify(
                {
                    'result' : 'success',
    ...
    productBoughtId: (req, res) => {
        connection.query('SELECT * from `order` inner join product on id_p = pid where id_u = ?', [req.params.id], async (err, rows) => {
            const response = await graphql(productsSchema, productsQuery, {produts: rows});
            res.setHeader('Content-Type', 'application/json');
            res.status(200).send(JSON.stringify(
                {
